...
class VOCSegmentation(BaseDataset):
    ...
    def __init__(self, args, base_dir, split, load_embedding, w2c_size, weak_label, unseen_classes_idx_weak, transform):
        super().__init__(args, base_dir, split, load_embedding, w2c_size, weak_label, unseen_classes_idx_weak, transform)

        self._image_dir = self._base_dir / "JPEGImages"
        self._cat_dir = self._cat_dir / "SegmentationClass"
    ...
    def transform_val(self,sample):
        composed_transforms = transforms.Compose(
            [
                FixScale(crop_size=self.args.crop_size),
                Normalize(mean=(0.485, 0.456, 0.406), std=(0.229, 0.224, 0.225)),
                ToTensor(),
            ]
        )
